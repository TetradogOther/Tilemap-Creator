	...
	partial class MainForm
	{
		...
		private void pTilemap_MouseMove(object sender, MouseEventArgs me)
		{
			...
			lPosition.Text = rModeTilemap.Checked ?
				String.Format("Tilemap: ({0}, {1})",tilemapMouseCurrent.X,tilemapMouseCurrent.Y) :
				String.Format("Palettemap: ({0}, {1})",tilemapMouseCurrent.X,tilemapMouseCurrent.Y);

			if (tilemap == null)
				return;

			var mousedTile = tilemap[tilemapMouseCurrent];
			lTile.Text = String.Format("Tile: {0}",mousedTile.TilesetIndex);
			lPalette.Text = String.Format("Palette: {0}",mousedTile.PaletteIndex);
			...
			else if (me.Button == MouseButtons.Right)
			{
				...
				lTilesetSelection.Text = rModeTilemap.Checked ?
					String.Format("({0}, {1}) to ({2}, {3})",tilesetSelection.X,tilesetSelection.Y,tilesetSelection.X + tilesetSelection.Width - 1 ,tilesetSelection.Y + tilesetSelection.Height - 1):
					String.Format("{0}",paletteSelection);
